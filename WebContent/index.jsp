					...
					<form method="post" class="form-horizontal" action="demo">
						<fieldset>
							<div class="form-group row">
								<div class="col-lg-10">
									<label for="questionTypeHealth">Health </label><input type="radio" name="questionType" value="health" id="questionTypeHealth">
									<label for="questionTypeTravel">Travel </label><input type="radio" name="questionType" value="travel" id="questionTypeTravel">
								</div>
								<script>
// questionType is ${param["questionType"]}
// questionType2 is ${param['questionType']}
// isTravel is ${param['questionType'] == 'travel'}
<c:choose>
	<c:when test="${param['questionType'] == 'travel'}">
		document.getElementById('questionTypeTravel').checked = true;
	</c:when>
	<c:otherwise>
		document.getElementById('questionTypeHealth').checked = true;
	</c:otherwise>
</c:choose>
								</script>
								<div class="col-lg-10">
				...
				<c:if test="${not empty questionText }">
					...
					<div class="well">
						<c:if test="${not empty answers }">
							...
							<div class="form-group row">
					        	<div class="col-lg-10">
								<p class="answer${answer.answerClass}">${answer.text}</p>
								</div>
					        	<div class="col-lg-2">
								<p class="answer${answer.answerClass}" style="text-align:center;" >${answer.confidence}</p>
								</div>
					        </div>
					        </c:forEach>
						</c:if>

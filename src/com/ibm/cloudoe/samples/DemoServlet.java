...
public class DemoServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String apiType = "healthcare";
		if (questionType.equals("travel")) {
			apiType = "travel";
		}
		req.setAttribute("questionType", apiType);
    	...
    	try {
    		...
    		URI serviceURI = new URI(baseURL+ "/v1/question/" + apiType).normalize();
	...
	private List<Map<String,String>> formatAnswers(String resultJson) {
		...
		try {
			...
			for(int i = 0; i < answers.size();i++) {
				...
				map.put("confidenceVal", Double.toString(p));
				p = Math.floor(p * 100);
				map.put("confidence",  Double.toString(p) + "%");
				String answerClass = "Bad";
				if (p > 75.0) {
					answerClass = "Best";
				} else if (p > 50.0) {
					answerClass = "Good";
				...
				map.put("answerClass", answerClass);
